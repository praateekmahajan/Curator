...
def test_terminal_row_failure_is_written_and_checkpointed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    from nemo_curator.backends.base import BaseStageAdapter
    from nemo_curator.tasks import EmptyTask
    from nemo_curator.utils.resumability_actor import ResumabilityActor
    from tutorials.text.manifest_inference import inference
    ...
    assert rows[1]["updated_qwen_metadata"]["failed_reason"] == ("empty_completion: Completion answer is empty")
    first_mtime = outputs[0].stat().st_mtime_ns
    assert attempt("second") == 0
    assert outputs[0].stat().st_mtime_ns == first_mtime
